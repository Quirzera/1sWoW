...
Процедура БроситьКубики(Команда)
	...
	ПутьККартинкеПодземелия = БроситьКубикиНаСервере(ПараметрыПолучения);
	
	МассивПутейККартинкам = новый Массив;
	МассивПутейККартинкам.Добавить(ПутьККартинкеПодземелия);
	
	ПараметрыОткрытия = новый Структура;
	ПараметрыОткрытия.Вставить("МассивПутейККартинкам",МассивПутейККартинкам);
	
	ОткрытьФорму("ОбщаяФорма.Kiur_ФормаОтображенияИзображений",ПараметрыОткрытия,ЭтаФорма,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
...
Функция БроситьКубикиНаСервере(Знач ФиксированныеПараметры, МодифицируемыеПараметры = Неопределено)
	...
	|	WoW_СезонныеПодземелья.Подземелье.ОсновноеИзображение КАК Подземелье
	|ИЗ
